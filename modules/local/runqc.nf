process RUNQC {
    ...
    output:
    tuple val(meta), path("${meta.id}_qc_reports/*"), emit: qc_reports
    ...
    mv *.png *.json "${meta.id}"_qc_reports
    ...
    """
    mkdir -p "${meta.id}"_qc_reports
    touch "${meta.id}"_qc_reports/${prefix}_qc_report.html
